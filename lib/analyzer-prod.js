...
function technicalLabel(value) {
  ...
  return !s || /\b(?:seçenek|secenek|seçim|secim|option|outcome|selection|pick)\s*[#:_-]?\s*\d+\b/i.test(s)
    || /\bmarket\s*\d+\b/i.test(s)
    || /^\d{3,}$/.test(s);
}
function outcomeLabel(o) {
  const current = String(o?.label || '').trim();
  if (!technicalLabel(current)) return current;
  const source = String(o?.sourceLabel || '').trim();
  return technicalLabel(source) ? '' : source;
}
function verified(o) { return (o?.labelVerified === true || Boolean(o?.sourceLabel)) && Boolean(outcomeLabel(o)); }
...
function storyReason(c, role, main, p, match) {
  ...
  if (c.market.kind === 'result_total_combo')
    return `NEÇ düşüncesi: ${c.outcome.label} hem maç sonucunu hem de toplam gol şartını aynı anda ister. Ana yön ${lead} ile uyumlu olsa da iki koşul birden gerektiği için yüksek risklidir ve ana seçim değil sürpriz senaryodur.`;
  if (c.market.kind === 'result_btts_combo')
    return `NEÇ düşüncesi: ${c.outcome.label} maç sonucu ile karşılıklı gol şartını aynı kupon ayağında birleştirir. Ana yön ${lead} ile uyumlu olsa da iki şart gerektiği için sürpriz seviyesinde değerlendirilir.`;
  if (c.market.kind === 'total_btts_combo')
    return `NEÇ düşüncesi: ${c.outcome.label} toplam gol ve karşılıklı gol koşullarını birlikte ister. Sonuç tarafına bağlı değildir ama iki ayrı gol şartı taşıdığı için yüksek risklidir.`;
...
function desirability(c, role) {
  ...
  const kindScore = kind === 'htft' ? 10 : kind === 'goal_range' ? 9 : kind === 'result_total_combo' ? 8.8 : kind === 'result_btts_combo' ? 8.6 : kind === 'total_btts_combo' ? 8.4 : kind === 'total_goals' ? 8 : kind === 'btts' ? 7 : kind === 'first_goal_team' ? 6 : 4;
...
function analyzeMatch(match, form) {
  ...
  for (const market of match.markets || []) {
    for (const outcome of market.outcomes || []) {
      const odds = Number(outcome.odds), label = outcomeLabel(outcome);
      if (odds > 1 && verified(outcome) && label) all.push({ market, outcome: { ...outcome, label }, odds });
    }
  }
  ...
  for (const [label, role, confidence] of plan) {
    ...
    if (!best) {
      result.picks[idx] = {
        ...
        reason: `NEÇ düşüncesi: ${label} için ana maç hikâyesiyle tutarlı, gerçek adı doğrulanmış uygun oran bulunamadı. Sırf beşinci kart dolsun diye anlaşılmayan veya ters seçim üretmiyorum.`
      };
      continue;
    }
