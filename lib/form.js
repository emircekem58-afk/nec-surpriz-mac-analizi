...
async function fetchFootballForm(home, away) {
  ...
  try {
    const res = await fetch(url, { headers: {'Accept':'application/json','User-Agent':'Mozilla/5.0 NEC-Analiz/2.4'}, cache:'no-store', signal:ctrl.signal });
    if (!res.ok) return null;
    const data = await res.json();
    const events = Array.isArray(data?.events) ? data.events : [];
    const h = summarize(events, home), a = summarize(events, away);
    if (!h && !a) return null;
    return { home:h, away:a, source:'ESPN public scoreboard', coverage:'best-effort', htftCoverage:'only when half-time detail is present' };
  } catch { return null; }
