...
function completedEvent(ev){return ev?.status?.type?.completed===true || String(ev?.status?.type?.state||'').toLowerCase()==='post';}
function halfTimeScore(comp){
  ...
  if(Number.isFinite(h1)&&Number.isFinite(a1))return {home:h1,away:a1};
  return null;
}
function streak(rows){
  if(!rows.length)return '';
  const first=rows[0].result;
  let n=0; for(const r of rows){if(r.result===first)n++;else break;}
  return `${first}${n}`;
}
function summarizeRows(rows){
  const played=rows.length;
  if(!played)return null;
  const goalsFor=rows.reduce((s,x)=>s+x.gf,0),goalsAgainst=rows.reduce((s,x)=>s+x.ga,0);
  const wins=rows.filter(x=>x.result==='W').length,draws=rows.filter(x=>x.result==='D').length,losses=rows.filter(x=>x.result==='L').length;
  return {
    ...
    avgGF:goalsFor/played,avgGA:goalsAgainst/played,
    pointsPerGame:(wins*3+draws)/played,
    recent:rows.slice(0,5).map(x=>x.result).join('-'),
    recentScores:rows.slice(0,5).map(x=>`${x.gf}-${x.ga}`),
    streak:streak(rows)
  };
}
function summarize(games,teamName){
  ...
  for(const ev of games){
    ...
    const ht=halfTimeScore(comp),htState=ht?scoreState(ht.home,ht.away):null,ftState=scoreState(hs,as);
    rows.push({
      date:ev?.date||null,gf:my,ga:op,opponent:opp?.team?.displayName||'',
      result:my>op?'W':my<op?'L':'D',over25:my+op>2.5,btts:my>0&&op>0,role,
  ...
  const roleStats={home:summarizeRows(last.filter(x=>x.role==='home')),away:summarizeRows(last.filter(x=>x.role==='away'))};
...
async function fetchFootballForm(home,away){
  ...
  const end=new Date(),start=new Date(end.getTime()-120*86400000),ranges=[];
  for(let cursor=new Date(start);cursor<end;cursor=new Date(cursor.getTime()+30*86400000)){
    const rEnd=new Date(Math.min(end.getTime(),cursor.getTime()+29*86400000));
  ...
  try{
    const settled=await Promise.allSettled(ranges.map(async([from,to])=>{
      ...
      const res=await fetch(url,{headers:{Accept:'application/json','User-Agent':'Mozilla/5.0 NEC-Analiz/3.1'},cache:'no-store',signal:ctrl.signal});
      if(!res.ok)return [];
    ...
    for(const batch of settled){
      ...
      for(const ev of batch.value||[])byId.set(String(ev?.id||ev?.uid||`${ev?.date}-${ev?.name||''}`),ev);
    }
    ...
    const value={
      ...
      coverage:'best-effort / 120-day lookup',
      completedOnly:true,
      htftCoverage:'only when half-time score is present'
    };
    FORM_CACHE.set(key,{at:Date.now(),value});
    return value;
  }catch(e){
    console.warn('[form]',e?.message||e);
    return null;
