...
function emergencyAnalysis(match, message) {
  ...
  return {
    ...
    scenario: {
      ...
      summary: 'Bağımsız form kaynağı bu istekte cevap vermedi. Maç ekranı kapatılmadı; yalnızca doğrulanmış bülten oranlarıyla sınırlı bir görünüm gösteriliyor.',
      formText: 'Takım form verisi geçici olarak alınamadı. Veri uydurulmadı.',
      confidence: 18,
      specialComment: main ? `NEÇ Özel geçici seçim: ${main.market} — ${main.selection} (${main.odds.toFixed(2)}). Bu seçim yalnızca bülten fiyatına dayanır; form teyidi beklenir.` : 'Bu maç için güvenilir ana market çözümlenemedi.',
      specialOdds: main?.odds ?? null,
      specialMarket: main?.market ?? null,
      specialSelection: main?.selection ?? null,
      av: 0
    },
    picks: main ? [{
      label:'Ana Senaryo', available:true, tag:'GEÇİCİ', market:main.market, selection:main.selection, odds:main.odds,
      av:0, avText:'0.0', confidencePct:18, model:null, fair:null, recommended:false,
      reason:'Takım form kaynağına ulaşılamadığı için yalnızca gerçek bülten fiyatı gösteriliyor; zıt veya uydurma ek tahmin üretilmedi.'
    }] : [],
    insights:[
      {title:'⚠️ Veri Kaynağı',text:message || 'Bağımsız form servisi geçici olarak yanıt vermedi.'},
      {title:'🧭 Tutarlılık',text:'Form verisi yokken birbiriyle çelişen alternatif tahminler üretilmez.'}
    ],
  ...
  try{
    ...
    try{
      ...
      try{
        ...
        return res.status(200).json(emergencyAnalysis(match, e2?.message || e?.message));
      }
    }
  }catch(e){
